...
def rotate_vector(vector, axis, angle):
    ...
    x = (
        (vector[0] * (math.cos(angle) + ((axis[0] ** 2) * (1 - math.cos(angle)))))
    ...
    y = (
        (vector[0] * ((axis[1] * axis[0] * (1 - math.cos(angle))) + (axis[2] * math.sin(angle))))
        + (vector[1] * (math.cos(angle) + ((axis[1] ** 2) * (1 - math.cos(angle)))))
        + (vector[2] * ((axis[1] * axis[2] * (1 - math.cos(angle))) - (axis[0] * math.sin(angle))))
    )
    z = (
        (vector[0] * ((axis[2] * axis[0] * (1 - math.cos(angle))) - (axis[1] * math.sin(angle))))
