...
if passed_mds == 0:
    ...
    forcefield_solvate = app.ForceField(
        main_forcefield, water_model_solvate
    )  # specify forcefield and water model
    ...
    app.PDBFile.writeFile(
        modeller.topology, modeller.positions, open(directory + pdb_file + "_solvated.pdb", "w")
    )
    # make directories
...
for counter in range(passed_mds, number_mds):
            ...
            if atom.residue.name not in non_protein_resnames:
                if atom.element is not None:
                    if atom.element.name is not "hydrogen":
                        force.addParticle(
                            index, modeller.positions[index].value_in_unit(unit.nanometers)
                        )
    ...
    simulation.reporters.append(
        app.DCDReporter(directory + "mds/" + str(counter) + ".dcd", 2500, enforcePeriodicBox=False)
    )
    simulation.reporters.append(
        app.StateDataReporter(
            stdout, 2500, step=True, potentialEnergy=True, temperature=True, speed=True
        )
    )
    simulation.step((md_length * 1000000) / 2.0)
    os.system(
        "{0} -f mds/0.pdb mds/{1}.dcd -dispdev text -e md_conversion.tcl -eofexit -args {1}".format(
            VMD, counter
        )
    )
