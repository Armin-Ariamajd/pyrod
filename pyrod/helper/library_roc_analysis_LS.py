...
def area_under_the_curve(
    actives_scores, decoys_scores, number_of_actives, number_of_decoys, fraction
):
    tp, fp = 0, 0
    auc_collector = 0
        ...
        if len(actives_scores) > 0:
            ...
            if len(decoys_scores) > 0:
                if actives_scores[-1] > decoys_scores[-1]:
                    auc_collector += auc_step(
                        fp, auc_tp_step, number_of_actives, number_of_decoys, fraction
                    )
                    tp += 1
...
def enrichment_factor(score_array, number_of_actives, number_of_decoys, fraction):
    ...
    return round(
        (tp / (tp + fp)) / (number_of_actives / (number_of_actives + number_of_decoys)), 1
    )
...
def roc_analyzer(current_file, directory, number_of_actives, number_of_decoys):
    ...
    if len(decoys_scores) > 0:
        score_array = np.vstack(
            (np.array([[x, 1] for x in actives_scores]), np.array([[x, 0] for x in decoys_scores]))
        )
    else:
    ...
    for fraction in [0.01, 0.05, 0.1, 1]:
        EFs.append(
            str(enrichment_factor(score_array, number_of_actives, number_of_decoys, fraction))
        )
        AUCs.append(
            str(
    ...
    print(
        "\rPharmacophore {0}: EF1={1} EF10={3} AUC10={7} AUC100={8} Actives={9}%".format(
            *result[:-1]
        )
    )
    return result
...
with open("/".join([directory, output_file_name]), "w") as result_file:
    ...
    while current_file <= last_file:
        ...
        with suppress_stdout():
            subprocess.run(
                "{0} -q {1}.pml -d {2}:active -o {1}.sdf -l {1}.log -C {3} -M {4}".format(
                    iscreen_path, file_name, actives, max_cpus, max_mem
                ),
            ...
            with open("{}.log".format(file_name, "r")) as log_file:
                if "iscreen finished successfully" in log_file.read():
                    ...
                    for line in log_file.readlines():
                        if "Using database" in line:
                            try:
                                ...
                                print(
                                    "\rNumber of actives in database cannot be found! Please check {}.log".format(
                                        file_name
                                    )
                                )
                                sys.exit()
                    if number_of_actives > 0:
                        ...
                        print(
                            "\rNumber of actives in database is found to be 0! Please check {0}.log and {1}".format(
                                file_name, actives
                            )
                        )
                        sys.exit()
        ...
        if roc_analysis:
            with suppress_stdout():
                subprocess.run(
                    "{0} -q {1}.pml -d {2}:active,{3}:inactive -o {1}.sdf -l {1}.log -R {1}.png ".format(
                        iscreen_path, file_name, actives, decoys
                    )
                    + "-C {0} -M {1}".format(max_cpus, max_mem),
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    shell=True,
                )
                            ...
                            if "Using database" in line:
                                if database_counter == 0:
                                    try:
                                        ...
                                        print(
                                            "\rNumber of actives in database cannot be found! Please check "
                                    ...
                                    try:
                                        ...
                                        print(
                                            "\rNumber of decoys in database cannot be found! Please check "
                                            "{}.log".format(file_name)
                                        )
                                        sys.exit()
                        if number_of_actives > 0 and number_of_decoys > 0:
                            result = roc_analyzer(
                                current_file, directory, number_of_actives, number_of_decoys
                            )
                            ...
                            print(
                                "\rNumber of actives or decoys in databases is found to be 0! Please check "
