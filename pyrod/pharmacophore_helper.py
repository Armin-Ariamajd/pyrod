...
def get_core_tolerance(position, tree, scores, maximal_score):
    ...
    for tolerance in [
        x / 10
        for x in range(
            int(minimal_cutoff * 10), int((maximal_cutoff * 10) + 1), int(step_size * 10)
        )
    ]:
...
def get_maximal_core_tolerance(indices, positions, tree, feature_scores, feature_maximum):
    ...
    for index in indices:
        tolerance, involved_indices = get_core_tolerance(
...
def get_partner_positions(feature_type, partner_positions_list):
    ...
    if feature_type in ["ha", "hd", "ha2", "hd2", "ai"]:
        partner_positions_array = np.array(
            [
                [x, y, z]
                for x, y, z in zip(
                    partner_positions_list[0::3],
                    partner_positions_list[1::3],
                    partner_positions_list[2::3],
                )
            ]
        )
        ...
        if feature_type in ["ha2", "hd2"]:
            partner_position2 = get_center(
                np.array([x for y, x in enumerate(partner_positions_array) if y not in used_list]),
                1.5,
            )[0]
            partner_positions.append(partner_position2)
    elif feature_type == "hda":
        partner_positions_array_list = [
            np.array([[x, y, z] for x, y, z in zip(w[0::3], w[1::3], w[2::3])])
            for w in partner_positions_list
        ]
        partner_positions_array1 = np.array(
            [
                [x, y, z]
                for x, y, z in zip(
                    partner_positions_list[0][0::3],
                    partner_positions_list[0][1::3],
                    partner_positions_list[0][2::3],
                )
            ]
        )
        partner_positions_array2 = np.array(
            [
                [x, y, z]
                for x, y, z in zip(
                    partner_positions_list[1][0::3],
                    partner_positions_list[1][1::3],
                    partner_positions_list[1][2::3],
                )
            ]
        )
