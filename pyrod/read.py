    ...
    with open(path, "r") as pharmacophore_file:
        for line in pharmacophore_file.readlines():
            if line[:6].strip() == "ATOM":
                ...
                if "P" in line[12:16]:
                    feature_list = [
                        feature for feature in pharmacophore if feature[0] == int(line[22:26])
                    ]
                    pharmacophore = [
                        feature for feature in pharmacophore if feature[0] == int(line[22:26])
                    ]
...
def pml_feature_point_or_volume(feature_list, feature):
    ...
    feature_list[3] = [
        float(feature.find("position").attrib["x3"]),
...
def pml_feature_vector(feature_list, feature):
    ...
    feature_list[3] = [
        float(feature.find("origin").attrib["x3"]),
        float(feature.find("origin").attrib["y3"]),
        float(feature.find("origin").attrib["z3"]),
    ]
    feature_list[4] = float(feature.find("origin").attrib["tolerance"])
    feature_list[5] = [
        [
            float(feature.find("target").attrib["x3"]),
            float(feature.find("target").attrib["y3"]),
    ...
    if feature.attrib["name"] == "HBA":  # switch to acceptor
        feature_list[3], feature_list[4], feature_list[5], feature_list[6] = [
            feature_list[5][0],
            feature_list[6],
            [feature_list[3]],
            feature_list[4],
...
def pml_feature_plane(feature_list, feature):
    ...
    feature_list[5] = [
        [
            float(feature.find("normal").attrib["x3"]),
            float(feature.find("normal").attrib["y3"]),
...
def merge_pyrod_hydrogen_bonds(pharmacophore, logger):
    ...
    valid_features = [
        feature for feature in pharmacophore if feature[1] not in ["ha2", "hd2", "hda", "ev"]
    ]
    exclusion_volumes = [feature for feature in pharmacophore if feature[1] == "ev"]
    pyrod_hydrogen_bonds = [
        feature for feature in pharmacophore if feature[1] in ["ha2", "hd2", "hda"]
    ]
