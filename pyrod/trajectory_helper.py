...
This module contains helper functions mainly used by the trajectory module.
"""
    ...
    for resname in lookup.valid_resnames:
        if selection.size == 0:
            ...
            selection = np.concatenate(
                (selection, topology[(topology["resname"] == resname)]), axis=0
            )
    if selection.size > 1:
            ...
            if name in lookup.hi_sel_dict[resname]:
                if resname != "CYS":
                    ...
                    if (
                        len(
                            main_atoms[
                                (main_atoms["resid"] == resid) & (main_atoms["name"] == "HG")
                            ]
                        )
                        == 1
                    ):
                ...
                if resname == "HIS":
                    if (
                        len(
                            main_atoms[
                                (main_atoms["resid"] == resid)
                                & ((main_atoms["name"] == "HD1") | (main_atoms["name"] == "HE2"))
                            ]
                        )
                        != 1
                    ):
...
def buriedness(center_position, positions):
    ...
    while len(positions) > 0:
        if origin_position is None:
            ...
            if position_angle_test(
                position_to_test, center_position, positions_used, angle_cutoff
            ):
